...
{
    UNNotificationContent*              content     = (__bridge UNNotificationContent*)contentPtr;
    NSString*                           jsonStr     = [content.userInfo objectForKey:kUserInfoKey];
...
NPBINDING DONTSTRIP void NPNotificationContentSetUserInfo(NPIntPtr contentPtr, const char* jsonStr)
{
    ...
    if(jsonStr != nil)
    {
        NSString* json = NPCreateNSStringFromCString(jsonStr);
        [data setObject:json forKey:kUserInfoKey];
    }
    
    [content setUserInfo:data];
}
...
NPBINDING DONTSTRIP void NPNotificationContentSetSoundName(NPIntPtr contentPtr, const char* soundName)
{
    ...
    NSString* str = NPCreateNSStringFromCString(soundName);
    str = [str stringByReplacingOccurrencesOfString:@".wav" withString:@""];
    if (![[NSBundle mainBundle] pathForResource:str ofType:@"wav"]) {
          NSLog(@"The path could not be created.");
          return;
     }
    else{
        NSLog(@"Founddddd file %@", str);
    }
}
